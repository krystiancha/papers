...
with repetitive objects with very simple textures (fig.~\ref{fig:gazebo_vs_real}).
 Lighting system is also very simplified, without any 
...
There are different camera types used through the article. A \textbf{spherical camera} is a type of camera with a field of view that covers the entire sphere and a \textbf{regular camera} is a type of camera with a field of view close to $90$ degrees.
...
To achieve scaling $v_{max}$ with memory, one can utilise a pool of threads or processes.
Now an important problem is choosing which spherical images should be cached in RAM.
...
actual environment. Photos captured to sparse will result in big jumps when camera
