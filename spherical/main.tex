...
\todo{Można dopisać odniesienia do innych rozwiązań rzutujących panoramki}{}
...
\todo{Trzeba pokazać zdjęcia z różną siatką, chyba przy okazji testów}{sufficiently dense} the effect is 
satisfactory.
...
photo) only the rendering time is important. For our system it takes \todo{wpisać ile zajmuje}{xxx ms} to
render single VGA view. This allows us for simulating the camera with \todo{wstawić}{yyy FPS}. 

The other latency is a result of loading spherical photo from disc and decoding it. 
\todo[inline]{Dopisz o rozdzielczości zdjęcia i czasie ładowania oraz wprowadzanych przez to ograniczeniach
na maksymalną szybkość robota w symulacji. Tutaj można dopisać też coś o predykcji kierunku ruchu
...
\todo[inline]{Jeden akapit tekstu plus obrazki dotyczące doboru wielkości oczka siatki}
